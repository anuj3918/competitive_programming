function repeatedNumber(A) {
	var temp = {};
	// We will do bucketing of elements into ranges based on srt(n)
	// { '1': [ 1, 2, 3, 1 ], '2': [ 4, 5, 6 ], '3': [ 7 ] }
	var n = A.length - 1;
	var sqrtN = Math.ceil(Math.sqrt(n));
	for (var i = 0; i < A.length; i++) {
		var element = A[i];
		var range = Math.ceil(element / sqrtN);

		if (!temp[range]) {
			temp[range] = [element];
		} else {
			temp[range].push(element);
		}
	}
	// console.log(temp);
	for (i = 1; i <= sqrtN; i++) {
